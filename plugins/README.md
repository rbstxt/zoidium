...
manifest version when the bundle builds: bump `version` once and run
`pnpm run build:plugin-bundles`. Never hand-edit per-file cache-busting queries;
`pnpm run check:plugin-bundles` rejects a manifest whose URLs drifted.
  ...
  "resources": [
    {
      "id": "style",
      "type": "text",
      "source": "./plugins/example/example.css"
    }
  ]
}
```
...
Input is exposed to GLSL as a `sampler2D`, like an image input, and its selector can choose
the Source, Masks, or Effects & Masks stage of the selected layer. The first
implementation targets top-level video tracks and stores references as stable track IDs,
so reordering tracks does not retarget a source. The runtime builds an isolated capture
for the selected track and reuses it for the effect, material, or shader during the current
frame. Hidden video tracks are still captured. Self-references and graph cycles are
