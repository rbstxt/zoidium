...
`zoidium/direct-download.js`が捕捉します。動画・画像の完了画面はそれぞれ固有の
Blobとファイル名を保持するため、後続のレンダーやプロジェクト保存に置き換わりません。
レンダーworkerとアーカイブworkerが共有する出力パスへの操作は、タブをまたいで
直列化します。別ページや新しいウィンドウは開きません。
